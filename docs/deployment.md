...
| `PROLOG_RESULTS_PASSWORD_CHANGE` | dotted path to `(email, current, new) -> None`, raising `results.PasswordRefused` with the host's own reasons: a reader changing their own password on the report page. Unset means the page does not offer to |
| `PROLOG_RESULTS_AUTH` | dotted path to `(email, password) -> ResultsViewer | None`: who may read a survey's results at `/s/<slug>/report`. The host's own login path is the only one that runs — PROlog holds no accounts and hashes no passwords. Unset (the default) means the page offers no way in |
| `PROLOG_MACHINE_LANGUAGES` | languages this deployment offers as machine translations, e.g. `es,pt`. Empty (the default) keeps DEF-5's gate exactly as it is: a version offering an unreviewed language cannot be activated. Naming a language is a statement that respondents will read a machine translation of it — the runner discloses that to them — and is a different thing from `--allow-unreviewed`, which only means "I am previewing this" |
