...
| RUN-19 | **Results are read from the survey, not the platform.** A survey has a report page at `/s/<slug>/report` in its own theme: the numbers a fieldwork owner needs (respondents, completions, partials, completion rate, average time, responses per day, by language, and the question unfinished responses reached last) and one CSV download per version, labelled with the rows behind it — the answers and the addresses as separate files behind separate permissions, never joined. Who may read is the deployment's decision: `PROLOG_RESULTS_AUTH` names a host callable, whose own login path (hashing, lockout, audit) is the only one that runs; PROlog stores no accounts, passwords or tokens, and a reader is a session entry. A deployment that names none has no way in and says so. Exports stream, so a long instrument is never held in memory at once. |
...
| Q-11 | `email` | email input with the question's help copy in an info panel, optional consent tick boxes with a note beneath (CON-9); equal-weight Skip | never stored as an answer — see CON-3/CON-4; the answer row records only `{"provided": true|false}` and the consent keys ticked |
...
| CON-3 | **Contact capture** (`email` question with `store_separately: true`): the address is validated and stored in a contact table with the survey version and consent text shown, **without any reference to the response or its answers**. It is never returned by the API, never joined in exports, and never logged. The capture reply hands the browser a receipt that opens that contact row alone; presented with a new address it rewrites the row (a correction on the spot), and it lives nowhere but the browser's memory. This is the exception, not the default (CON-4): it produces a mailing list and nothing else — no account, no way for the participant to reach their own data, and no way to honour a later access or erasure request about their answers. A deployment choosing it should be able to say why. |
...
| CON-9 | **Separate consents with the address.** An `email` question may offer up to five consents (`config.consents`), each a sentence with its own tick box, none pre-ticked, an optional line above (`consents_label`); a note beneath (`consents_note`) says how to withdraw and links the legal pages. An address with nothing ticked is accepted unless the definition sets `consents_min`. The runner sends the ticked keys with the address and the server validates them against the definition. What is recorded is the wording as shown: with contact capture, on the contact row (dated to the day, unlinked); with identity capture, one row per consent against the response — key, text, language, timestamp, `withdrawn_at`. The response itself keeps only the keys beside its `provided` marker. Exports carry one `1`/`0` column per consent. |
| CON-10 | **Linked contact capture** (`email` question with `link_response: true`): the address is validated and stored on a row keyed by the response — one per response, with the notice shown, the consents ticked and a timestamp — so the answers can be found from the address and the address from the answers, with no account created. Never returned by the API and never in the response export; the contact export carries the response id, which is the join and the only place it exists. The same receipt, correction and removal as CON-3. A purged response takes the row with it; `withdraw_consent --erase-response` deletes the whole record on request. An instrument using it is not anonymous for those who give an address (CON-8). |
