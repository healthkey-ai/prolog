...
| Legal pages | `legal.privacy` `legal.terms` `legal.back` `legal.missing` `legal.noteBack` (`{{note}}`) — the link to a deployment's own notice and the page around it; the notice's *content* comes from the deployment's own Markdown file, never from a chrome string |
...
| Email step | `email.placeholder` `email.save` `email.skip` `email.saved` `email.invalid` `email.unavailable` `email.consentsRequired` `email.savedAs` `email.change` `email.cancel` `email.remove` |
| Report page | `report.eyebrow` `report.title` `report.signInIntro` `report.email` `report.password` `report.signIn` `report.signInFailed` `report.noSignIn` `report.signedInAs` (`{{label}}`) `report.signOut` `report.downloads` `report.downloadsNote` `report.downloadResponses` (`{{count}}`) `report.downloadAll` (`{{count}}`) `report.downloadContacts` (`{{count}}`) `report.versions` `report.version` `report.versionLabel` (`{{version}}`) `report.respondents` `report.completions` `report.partials` `report.completionRate` `report.averageTime` `report.byLanguage` `report.language` `report.byDay` `report.day` `report.started` `report.completed` `report.dropOff` `report.dropOffNote` `report.question` `report.responses` `report.beforeStarting` `report.noneYet` `password.show` `password.hide` `password.change` `password.current` `password.new` `password.confirm` `password.mismatch` `password.save` `password.changed` `password.close` — the results page at `/s/<slug>/report`, which only a reader the deployment recognises ever sees |
