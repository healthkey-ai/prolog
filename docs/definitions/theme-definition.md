...
| Report page | `report.eyebrow` `report.title` `report.signInIntro` `report.email` `report.password` `report.signIn` `report.signInFailed` `report.noSignIn` `report.signedInAs` (`{{label}}`) `report.signOut` `report.downloads` `report.downloadsNote` `report.downloadResponses` `report.downloadAll` `report.downloadContacts` `report.versions` `report.version` `report.versionLabel` (`{{version}}`) `report.respondents` `report.completions` `report.partials` `report.completionRate` `report.averageTime` `report.byLanguage` `report.language` `report.byDay` `report.day` `report.started` `report.completed` `report.dropOff` `report.dropOffNote` `report.question` `report.responses` `report.beforeStarting` `report.noneYet` — the results page at `/s/<slug>/report`, which only a reader the deployment recognises ever sees |
