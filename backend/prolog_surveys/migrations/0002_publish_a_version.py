...
has the name that says so. **That makes this migration one-way**: code from
before it writes ``published_at`` on every activation, which against this schema
stamps the freeze column instead. Do not run the previous release against a
database that has this migration — a rollback, or a not-yet-replaced worker
activating during a rolling deploy, would publish versions nobody published, and
there is no unpublish. Existing versions are left unpublished: a deployment
